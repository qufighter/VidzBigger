	...
	//		  		cel(" L o a d i n g  T o p  V i d e o s"),
  	...
  	function loadTopVidzURL(){
  		...
  		chrome.extension.sendRequest({'url':murl+(color_toggled?2:1)+(page?('&page='+page):''),'method':'GET','msgType':"G_xmlhttpRequest"}, function(response){
  	...
  	function loadFloatingUrl(ur){
  		...
  		chrome.extension.sendRequest({'url':ur,'method':'GET','msgType':"G_xmlhttpRequest"}, function(response){
	  		if(response.responseText && response.responseText.length > 0 ){
